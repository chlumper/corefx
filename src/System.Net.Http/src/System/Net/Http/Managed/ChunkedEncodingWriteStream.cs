...
namespace System.Net.Http
{
    internal sealed partial class HttpConnection : IDisposable
    {
        private sealed class ChunkedEncodingWriteStream : HttpContentWriteStream
        {
            ...
            public override Task WriteAsync(ReadOnlyMemory<byte> source, CancellationToken ignored) // CancellationToken is ignored as it comes from the handler itself while sending content to the server
            {
                if (source.Length == 0)
                {
                    ...
                    return _connection.FlushAsync(default);
                }
            ...
            private async Task WriteChunkAsync(ReadOnlyMemory<byte> source)
            {
                ...
                await _connection.FlushAsync(default).ConfigureAwait(false);
            }

            public override Task FlushAsync(CancellationToken ignored) => // CancellationToken is ignored as it comes from the handler itself while sending content to the server
                _connection.FlushAsync(default);
